...
Fields: `settingsAssetPath`, `activeEnvironment`, `activeBuildTarget`, `healthy`, `healthIssues`,
`defaultConfigKeys`, `environments[]`, `profiles[]`, `queues[]`. Each environment carries `id`,
`displayName`, `assetPath`, `active`, `addedDefines`, `removedDefines`, the override fields
(`productNameOverride`, `companyNameOverride`, `applicationIdentifierOverride`,
`applicationIconOverride` as an asset path), `variables` (`key=value`), `configKeys` and
`actionCounts`.
...
| `-bmkProductName <name>` | `""` clears the override |
| `-bmkCompanyName <name>` | `""` clears the override |
| `-bmkAppIdentifier <id>` | e.g. `com.studio.game.dev`; `""` clears |
| `-bmkIcon <Assets/path.png>` | application icon while this environment is active; `""` clears. Must import as a Texture2D, not a Sprite |
...
`-bmkEnv <id>`. Refuses to delete the last remaining environment while it is active.
